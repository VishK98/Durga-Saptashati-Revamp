...
<div class="data-panel">
    ...
    <table class="data-table paginated-table" id="membersTable">
        ...
        <tbody>
            ...
            <tr>
                <td colspan="6" style="text-align:center;padding:40px;color:#999;">No membership applications yet.</td>
            </tr>
            <?php else: ?>
                ...
                <td style="text-align:right;">
                    <div class="action-wrap">
                        ...
                        <div class="action-menu">
                            ...
                            <a href="javascript:void(0)" onclick="ajaxMemberAction('reject_member', <?= $m['id'] ?>)"><i
...
function viewMember(id) {
    ...
    rows.forEach(function(r) {
        if (r[1]) html +=
            '<tr><td style="padding:10px 14px;font-weight:600;color:#666;width:40%;border-bottom:1px solid #f0f0f0;">' +
            r[0] + '</td><td style="padding:10px 14px;color:#1a1b2e;border-bottom:1px solid #f0f0f0;">' + r[1] +
            '</td></tr>';
    });
    html += '</table>';
...
function exportMembers() {
    ...
    allMembers.forEach(function(m) {
        rows.push([
            ...
            '"' + (m.membership_type || '') + '"', '"' + (m.profession || '') + '"', m.payment_mode ||
...
function filterMembers() {
    ...
    rows.forEach(function(row) {
        ...
        var profession = (cells[4] ? cells[4].textContent : '').toLowerCase();
        var rowStatus = (cells[8] ? cells[8].textContent.trim().toLowerCase() : '');
        var rowType = row.getAttribute('data-type') || '';
        var rowPayment = row.getAttribute('data-payment') || '';

        var matchSearch = !search || name.indexOf(search) > -1 || email.indexOf(search) > -1 || mobile.indexOf(
            search) > -1 || profession.indexOf(search) > -1;
